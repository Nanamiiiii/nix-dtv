...
Recent commits commonly use `component: imperative summary`, such as `edcb: fix http port settings`; follow that pattern. Keep changes focused. Describe behavior changes, affected options, and validation commands/results in pull requests. Link relevant issues and update README examples when configuration changes.
